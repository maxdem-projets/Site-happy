...
async function run() {
  ...
  let articlesList = [];
  try {
    // Utilisation d'une évaluation sécurisée car le fichier est écrit par nous-mêmes
  ...
  articlesList.forEach(art => {
    ...
    // Si l'article était mis en avant (featured), on enlève l'étoile de Une pour le nouveau jour
  ...
  // Dictionnaire pour limiter à 3 articles max par catégorie pour aujourd'hui
  const categories = [
  ...
  let hasFlip = false;
  const storiesToProcess = rawStories.slice(0, 300); // Traiter un ensemble beaucoup plus large pour garantir notre quota de 24 + 1 articles

  for (const story of storiesToProcess) {
    // Vérifier si le titre est déjà dans notre base de données pour éviter les doublons
    ...
    // Le premier article retenu sera le "Flip" de la journée
    ...
    if (isGeneratingFlip) {
      prompt = `
        ...
        3. Rédige le contenu en français.
      ...
      prompt = `
        ...
        2. Si elle est positive, traduis et reformule l'information en français.
      ...
      if (parsedData.isPositive) {
        ...
        if (isGeneratingFlip) {
          ...
          const newArt = {
            ...
            image: "",
            offsetDays: 0,
            featured: true // C'est le Flip principal en vedette
          };
          ...
          // Ajouter un article de thématique standard (max 3 par thématique)
          const newArt = {
            id: articlesList.length + newArticles.length + 1,
            category: cat,
            title: parsedData.title,
        ...
        // Vérifier si nous avons complété tout le quota : 1 flip et 3 articles par thématique (total 25 articles)
  ...
  // Fusionner les nouveaux articles en tête de liste (les nouveaux articles du jour en premier)
  const updatedArticlesList = [...newArticles, ...articlesList];

  // Limiter la taille totale de l'historique à 200 articles pour stocker confortablement plusieurs jours
